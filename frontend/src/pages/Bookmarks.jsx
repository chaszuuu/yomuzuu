...
export default function Bookmarks() {
  ...
  return (
    <div style={{ background: "#080808", minHeight: "100vh", color: "white", fontFamily: "'Outfit', sans-serif", display: "flex", flexDirection: "column" }}>
      <style>{`
      ...
      {loading ? <TopbarSkeleton showCount /> : (
        <div className="bg-[#050505] border-b border-[#222222] px-4 sm:px-10 py-3 relative z-50 flex items-center gap-2">
          ...
          <span className="text-[#aaaaaa] text-[12px]">Bookmarks</span>
          {bookmarks.length > 0 && (
            <span className="text-[11px] text-[#333333] ml-auto tracking-widest uppercase">{bookmarks.length} {bookmarks.length === 1 ? "title" : "titles"}</span>
          )}
        </div>
      )}
      ...
      {loading ? <PageTitleSkeleton /> : (
        <div style={{ padding: "28px 40px 24px", borderBottom: "1px solid #111111" }}>
          ...
          <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 48, letterSpacing: 3, color: "#ffffff", lineHeight: 1 }}>Bookmarks</h1>
        </div>
      )}

      {/* CONTENT */}
      <div style={{ padding: "40px 40px 60px", flexGrow: 1 }}>
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 320, gap: 16 }}>
            ...
            <Link to="/" style={{ fontSize: 11, color: "#555555", letterSpacing: 2, textTransform: "uppercase", textDecoration: "none", borderBottom: "1px solid #222222", paddingBottom: 2, transition: "color 0.2s" }} onMouseEnter={e => e.currentTarget.style.color = "#ffffff"} onMouseLeave={e => e.currentTarget.style.color = "#555555"}>Browse Manga →</Link>
          </div>
        ) : (
